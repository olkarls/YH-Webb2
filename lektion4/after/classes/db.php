<?php
  ...
  class Db {
    ...
    public function updateItem($id, $title, $text, $category_id) {
      $data = array($title, $text, $category_id, $id);
      $sth = $this->dbh->prepare("update items set title = ?, text = ?, category_id = ? where id = ?");
      $sth->execute($data);
